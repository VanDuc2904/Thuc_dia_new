const tests = {
    ...
    test3: {
        ...
        description: {
            title: "Topic: Protecting the Environment",
            sections: [
    ...
    test10: {
        ...
        description: {
            title: "Topic: The Use of the Internet",
            sections: [
    ...
    test12: {
        ...
        description: {
            title: "Topic: Listening to Music",
            sections: [
    ...
    test11: {
        ...
        description: {
            title: "Topic: Environmental Problem",
            sections: [
    ...
    test5: {
        ...
        description: {
            title: "Topic: A Place for Studying",
            sections: [
    ...
    test6: {
        ...
        description: {
            title: "Topic: Outdoor Activities",
            sections: [
...
function loadContent() {
    ...
    test.questions.forEach((q, index) => {
        ...
        div.innerHTML = `
            ...
            `).join("")}
            <button onclick="checkAnswer(${index}, '${testSelect}')">Check Answer</button>
            <p id="result${index}"></p>
        `;
        questionsContainer.appendChild(div);
    });

    // Load description (Part 14)
    const descDiv = document.createElement("div");
    descDiv.className = "description-container";
    descDiv.innerHTML = `
        <h2>${test.description.title}</h2>
        ${test.description.sections.map(section => `
    ...
    // Load dialogue (Part 15)
    const diagDiv = document.createElement("div");
    diagDiv.className = "dialogue-container";
    diagDiv.innerHTML = `
        <h2>${test.dialogue.title}</h2>
        <p>${test.dialogue.conversation.replace(/\n/g, "<br>")}</p>
        <h3>Key Points:</h3>
        <ul>${test.dialogue.keypoints.map(point => `<li>${point}</li>`).join("")}</ul>
    `;
    dialogueContainer.appendChild(diagDiv);
}

function checkAnswer(index, testId) {
    ...
    const correctAnswer = tests[testId].questions[index].answer;
